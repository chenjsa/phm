...
(function() {
	...
	var projectPath = location.pathname
	var mm = projectPath.split("/");
	projectPath = '/' + mm[1];
	Ext.apply(Ext, {
...
/*
 * \u589e\u52a0\u4e86 projectPath \u548c\u4fee\u6539Ext.BLANK_IMAGE_URL\u7684\u8def\u5f84
 */
 ...
 *\u4e0b\u62c9\u6811\u5355\u9009\u6846
 */
Ext.form.BmComBoxTree = function(config) {
	Ext.apply(this, config);

	this.addEvents(

			"treeClick", "treeExpand"

	);

	Ext.form.BmComBoxTree.superclass.constructor.call(this);
};
Ext.form.BmComBoxTree = Ext.extend(Ext.form.ComboBox, {
			allowBlank : false,
			path : null,
			blankText : '\u4e0d\u80fd\u4e3a\u7a7a',
			...
			tree : null,
			initComponent : function() {
				Ext.form.BmComBoxTree.superclass.initComponent.call(this);
				var treedata1 = new Ext.tree.TreeLoader({

							url : this.path,
							listeners : {
						beforeload : function(This, node, response) { 
 													var	l_dwid=node.attributes.id;  
													var pp=  {
															dwid1 :l_dwid 
														}
														treedata1.baseParams =pp  
													},
				...
				this.fireEvent("treeClick", tree);
				this.fireEvent("treeExpand", tree);

			}

		});
Ext.reg('bmcomboxtree', Ext.form.BmComBoxTree);
...
Ext.grid.ButtonColumn.prototype = {
	init : function(grid) {
		...
		switch (this.processSign) {
			...
			case 'save' :
				...
				break;
			default :
				Ext.Msg.alert('\u63d0\u793a', 'process\u503c\u8d85\u51fa\u8303\u56f4!<br>\u6b63\u5e38\u8303\u56f4:<br> del,add,save');
		}
	...
	onMouseDown : function(e, t) {
		if (t.className
				&& t.className.indexOf('x-grid3-cc-operator' + this.id) != -1) {
			e.stopEvent();
			...
			switch (this.processSign) {
				case 'del' :
					Ext.MessageBox.confirm('\u63d0\u793a', '\u786e\u5b9a\u5220\u9664\u8bb0\u5f55', showResult);
				...
				case 'add' :
					/*
					 * var p = new Plant({ common: 'New Plant 1', light: 'Mostly
					 * Shade', price: 0, availDate: (new Date()).clearTime(),
					 * indoor: false }); this.grid.stopEditing();
					 * this.grid.store.insert(0, p); this.grid.startEditing(0,
					 * 0);
					 */
					break;
				case 'save' :
	...
	renderer : function(v, p, record) {
		...
		// return '<div class="remove' + ' x-grid3-cc-' + this.id
		// + '">&#160;</div>';
		return '<div class="' + this.processSign + ' x-grid3-cc-operator'
				+ this.id + '">&#160;</div>';
 ...
 * \u5206\u9875\u663e\u793a\u5de5\u5177\u6761
 * */
Ext.PagingToolbar.PagNum = Ext.extend(Ext.PagingToolbar, {
			displayInfo : true,
			displayMsg : '\u5f53\u524d\u8bb0\u5f55 {0} - {1} \u603b\u8bb0\u5f55\u6570 {2}',
			emptyMsg : "\u6ca1\u6709\u8bb0\u5f55",
			beforePageText : '\u7b2c',
...
Ext.reg('PagingToolbarPagNum', Ext.PagingToolbar.PagNum);
...
Ext.UploadButton = Ext.extend(Ext.Button, {
	text : null,/* \u6309\u94ae\u6587\u672c */
	formEmptyText : null,/* form\u6587\u4ef6\u8def\u5f84\u4e3a\u7a7a\u662f\u663e\u793a */
	formFieldLabel : null,/* form\u6587\u4ef6\u6807\u7b7e\u6587\u672c */
	formButtonText : null,/* form\u6587\u4ef6\u6309\u94ae\u6587\u672c */
	disabled : true,
	fileType : null,/* \u4e0a\u4f20\u6587\u4ef6\u7c7b\u578b */
	fileSize : null,/* \u4e0a\u4f20\u6587\u4ef6\u5927\u5c0f */
	filePath : null,/* \u4e0a\u4f20\u6587\u4ef6\u8def\u5f84 */
	uploadPath : null,/* form\u63d0\u4ea4\u8def\u5f84 */
	returnFile : null,
	initComponent : function() {
		Ext.UploadButton.superclass.initComponent.call(this);
		this.addEvents("uploadAfter");
	},
	onClick : function(e) {
		var tem_UploadButton = this;
		var photoUpload;
		if (!photoUpload) {
			photoUpload = new Ext.FormPanel({
						labelAlign : 'left',
						labelWidth : 75,
						title : '\u8be6\u7ec6\u4fe1\u606f',
						collapsible : true,
						...
						items : [{
									...
									items : [new Ext.form.FileUploadField({
												id : 'form-file',
												emptyText : this.formEmptyText,
												fieldLabel : this.formFieldLabel,
												name : 'file',
												buttonCfg : {
													text : this.formButtonText
													// iconCls : 'upload-icon'
												}
											})]
								}]
		...
		var win;
		if (!win) {
			var tem_uploadPath = this.uploadPath;
			var tem_fileType = this.fileType;
			var tem_fileSize = this.fileSize;
			var tem_filePath = this.filePath;
			...
			win = new Ext.Window({
						...
						buttons : [{
							...
							disabled : false,
							handler : function() {
								var tt = Ext.get('form-file');
								var fileName = tt.getValue().split("\\"); 
								if (fileName.length == 1) {
									Ext.Msg.alert('\u63d0\u793a', '\u8bf7\u9009\u62e9\u4e0a\u4f20\u6587\u4ef6!');
									return;
								}
								if (photoUpload.getForm().isValid()) {

									photoUpload.getForm().submit({
										url : tem_uploadPath,
										waitMsg : '......\u6570\u636e\u5904\u7406\u4e2d.....',
										waitTitle : '\u6570\u636e\u4fdd\u5b58',
										params : {
											fileType : tem_fileType,
											fileSize : tem_fileSize,
											filePath : tem_filePath
										},
										success : function(re, v) {
											var jsonobject = Ext.util.JSON
													.decode(v.response.responseText);

											tem_UploadButton.returnFile = jsonobject;

											tem_UploadButton.fireEvent(
													"uploadAfter",
													tem_UploadButton);
											win.close();
										},
										failure : function(re, v) {
											var jsonobject = Ext.util.JSON
													.decode(v.response.responseText);
											Ext.Msg.alert("\u63d0\u793a", jsonobject.msg); // \u8fd4\u56de\u5931\u8d25
										}
									});
								}
							}
		...
		win.show(Ext.UploadButton);
		// e.preventDefault();
		// this.fireEvent("click", this, e);

	}
});
Ext.reg('UploadButton', Ext.UploadButton);
...
Ext.namespace('system.persistent');
/* \u5b58\u653e\u56fa\u5b9a\u503c\u4e0b\u62c9\u9009\u62e9\u6846 */
system.persistent.sex = [['\u7537', '\u7537'], ['\u5973', '\u5973']];
system.persistent.finsh = [['true', '\u5b8c\u6210'], ['false', '\u8fdb\u884c']];
system.persistent.wjmj = [[0, '\u516c\u5f00'], [1, '\u673a\u5bc6'], [2, '\u79d8\u5bc6'], [3, '\u7edd\u5bc6']];
/* \u5b58\u653e\u7248\u672c\u53f7 */
system.persistent.version = [
['20100507','1.\u9886\u5bfc\u6253\u5f00\u672c\u90e8\u95e8\u4e2d\u7684\u9886\u5bfc\u7b80\u4ecb\u65f6\uff0c\u9886\u5bfc\u6587\u7a3f\u663e\u793a\u672c\u4eba\u6587\u7a3f\u3002<br>'+
'2.\u9886\u5bfc\u6253\u5f00\u5176\u4ed6\u90e8\u95e8\u4e2d\u7684\u9886\u5bfc\u7b80\u4ecb\u65f6\uff0c\u9886\u5bfc\u6587\u7a3f\u663e\u793a\u5176\u4ed6\u90e8\u95e8\u6240\u5728\u5355\u4f4d\u9886\u5bfc\u7684\u5168\u90e8\u6587\u7a3f\u3002<br>'+
'3.\u5728\u9886\u5bfc\u6587\u7a3f\u548c\u90e8\u95e8\u6587\u4ef6\u7efc\u5408\u67e5\u8be2\u4e2d\u65b0\u589e\u9886\u5bfc\u67e5\u8be2'],
		[
				'20100504',
				'1.\u9886\u5bfc\u4e00\u5468\u5b89\u6392\u4e0a\u5348\u4e0b\u5348\u665a\u4e0a\u5bbd\u5ea6\u589e\u52a0<br>' + '2.\u4ee3\u9886\u5bfc\u8d77\u8349\u6587\u4ef6\u4e0d\u80fd\u8f93\u5165<br>'
						+ '3.\u4eba\u5458\u5f55\u5165\u4e2d\u6709\u4e24\u4e2a\u4e13\u4e1a<br>' + '4.\u4eba\u5458\u5f55\u5165\u65f6\u5206\u5de5\u548c\u57fa\u672c\u4fe1\u606f\u4e0d\u80fd\u540c\u65f6\u4fdd\u5b58<br>'
						+ '5.\u53ea\u80fd\u5728\u672c\u90e8\u95e8\u8fdb\u884c\u65b0\u589e\u3001\u5220\u9664\u3001\u4fdd\u5b58.\u5728\u5176\u4ed6\u90e8\u95e8\u53ea\u80fd\u67e5\u8be2!<br>'],
		[
				'20100409',
				'1.\u9886\u5bfc\u7b80\u4ecb\u4e2d\uff0c\u9886\u5bfc\u6587\u4ef6\u6539\u6210\u9886\u5bfc\u6587\u7a3f<br>'
						+ '2.\u5728\u9886\u5bfc\u7b80\u4ecb\u4e2d\uff0c\u65b0\u589e\u9886\u5bfc\u4e00\u5468\u5de5\u4f5c\u6807\u7b7e\u5305\u62ec:<br>   \u67e5\u8be2\u6761\u4ef6\u5305\u62ec:<br>   1.\u65e5\u671f\u65f6\u95f4\u6bb5<br>   2.\u9886\u5bfc;<br>'
						+ '3.\u5728\u90e8\u95e8\u7b80\u4ecb\u4e2d\uff0c\u65b0\u589e\u9886\u5bfc\u4e00\u5468\u5de5\u4f5c\u5f55\u5165\uff1a\u9886\u5bfc\u4eba\u5458\u3001\u65e5\u671f\u3001\u661f\u671f\u3001\u4e0a\u5348\u3001\u4e0b\u5348\u665a\u4e0a\u3001\u662f\u5426\u5b8c\u6210\u3001\u5f55\u5165\u4eba\u5458<br>'
						+ ' \u5f55\u5165\u65b9\u5f0f;\u8868\u683c\u5f0f\u5f55\u5165;\u6309\u65f6\u95f4\u6bb5\u548c\u9886\u5bfc\u65b0\u589e\u8bb0\u5f55;\u53ea\u6709\u5f55\u5165\u4eba\u5458\u5177\u6709\u4fee\u6539\u4fdd\u5b58\u529f\u80fd\uff0c\u5176\u4ed6\u53ea\u6709\u67e5\u770b;<br>'
						+ '4.\u5728\u9886\u5bfc\u7b80\u4ecb\uff0d\u9886\u5bfc\u6587\u7a3f\u548c\u90e8\u95e8\u7b80\u4ecb\uff0d\u90e8\u95e8\u6587\u4ef6:\u65b0\u589e\u53d1\u6587\u65f6\u95f4\u5b57\u6bb5\uff0c\u5f55\u5165\u65f6\u4e3a\u53ef\u9009\u9879\uff1b<br>\u6587\u4ef6\u67e5\u770b\u65b9\u5f0f\u65b0\u589e\u5728\u7ebf\u6253\u5f00\u548c\u4e0b\u8f7d\u6309\u94ae<br>'
						+ '5.\u4eba\u5458\u65b0\u589e\u5b57\u6bb5\uff1a\u6587\u4ef6\u5bc6\u7ea7\u6743\u9650<br>' + '6.\u65b0\u589e\u6587\u4ef6\u5bc6\u7ea7\u6743\u9650\u7f16\u7801<br>'
						+ '7.\u6587\u4ef6\u67e5\u8be2\u65f6\uff0c\u65b0\u589e\u6587\u4ef6\u5bc6\u7ea7\u6743\u9650\u6761\u4ef6<br>' + '\u7a0b\u5e8f\u5347\u7ea7\u6b65\u9aa4\uff1a<br>'
						+ '1.\u5efa\u8868weekWork\u548c\u5efa\u5e8f\u5217S_weekWork_id'],
		['20100330',
				'1.\u6587\u4ef6\u6807\u9898\u62c9\u5bbd.<br>' + '2.\u4ee3\u9886\u5bfc\u8d77\u8349\u6587\u4ef6\u4e2d\u9886\u5bfc\u987a\u5e8f\u964d\u987a<br>' + '3.\u5927\u56fe\u7247\u663e\u793a\u4e0d\u6b63\u5e38<br>']
		];
/** \u7cfb\u7edf\u4e0b\u62c9\u9009\u62e9\u6846 */
/**
 * \u63cf\u8ff0:\u5b8c\u6210\u72b6\u6001 <br>
 * \u76ee\u524d\u7528\u4e8e \u4e00\u5468\u5b89\u6392
 */
var finshStore = new Ext.data.SimpleStore({
	name : "finshStore",
	fields : ['id', 'name'],
	data : system.persistent.finsh
		// from states.js
	});
/**
 * \u63cf\u8ff0:\u5b8c\u6210\u72b6\u6001 <br>
 * \u76ee\u524d\u7528\u4e8e \u4e00\u5468\u5b89\u6392
 */
Ext.form.BmFinsh = Ext.extend(Ext.form.ComboBox, {
			allowBlank : false,
			emptyText : '\u8bf7\u9009\u62e9\u72b6\u6001',
			blankText : '\u4e0d\u80fd\u4e3a\u7a7a',
			fieldLabel : '\u72b6\u6001',
			...
			displayField : "name",
			initComponent : function() {
				Ext.form.BmFinsh.superclass.initComponent.call(this);
			}
		})
Ext.reg('bmFinshCombo', Ext.form.BmFinsh);
/** \u63cf\u8ff0:\u672c\u5355\u4f4d\u5168\u90e8\u9886\u5bfc */
var ldStore = new Ext.data.SimpleStore({
			name : "ldStore",
			url : Ext.projectPath,
			fields : ['id', 'name']
 ...
 * 2\u3001\u90e8\u95e8\u6587\u4ef6
 */
Ext.form.BmRyld = Ext.extend(Ext.form.ComboBox, {
			fieldLabel : '\u9886\u5bfc\u5217\u8868',
			emptyText : '\u8bf7\u9009\u62e9.....',
			store : ldStore,
			triggerAction : 'all',
			...
			displayField : "name",
			initComponent : function() {
				Ext.form.BmRyld.superclass.initComponent.call(this);
			}
		})
Ext.reg('bmRyldCombo', Ext.form.BmRyld);
/** \u63cf\u8ff0:\u672c\u5355\u4f4d\u5168\u90e8\u4eba\u5458 */
var bmRyStore = new Ext.data.SimpleStore({
			name : "bmRyStore",
			url : Ext.projectPath,
			fields : ['id', 'name']
 ...
 * 1\u3001\u4e00\u5468\u5b89\u6392\u4e2d\u7684\u5f55\u5165\u4eba\u5458
 */
Ext.form.BmRy = Ext.extend(Ext.form.ComboBox, {
			fieldLabel : '\u4eba\u5458\u5217\u8868',
			emptyText : '\u8bf7\u9009\u62e9.....',
			store : bmRyStore,
			triggerAction : 'all',
			...
			displayField : "name",
			initComponent : function() {
				Ext.form.BmRy.superclass.initComponent.call(this);
			}
		})
Ext.reg('bmRyCombo', Ext.form.BmRy);
 ...
 * \u76ee\u524d\u7528\u4e8e\u90e8\u95e8\u6587\u4ef6<br>
 */
Ext.form.BmWjlx = Ext.extend(Ext.form.ComboBox, {
			allowBlank : false,
			fieldLabel : '\u6587\u4ef6\u7c7b\u578b',
			emptyText : '\u8bf7\u9009\u62e9.....',
			store : filetypeStore,
			...
			displayField : "name",
			initComponent : function() {
				Ext.form.BmWjlx.superclass.initComponent.call(this);
			}
		})
Ext.reg('wjlxCombo', Ext.form.BmWjlx);
 ...
 * \u76ee\u524d\u7528\u4e8e\u4eba\u5458\u4fe1\u606f<br>
 */
Ext.form.BmSex = Ext.extend(Ext.form.ComboBox, {
			allowBlank : false,
			emptyText : '\u8bf7\u9009\u62e9\u6027\u522b',
			blankText : '\u4e0d\u80fd\u4e3a\u7a7a',
			fieldLabel : '\u6027\u522b',
			...
			hiddenName : 'sex',
			readOnly : true,
			valueField : "name",// \u5b9e\u9645\u5728\u8bf7\u6c42\u4e2d\u4f20\u9012\u7684\u503c
			displayField : "name",
			initComponent : function() {
				Ext.form.BmSex.superclass.initComponent.call(this);
			}
		})
Ext.reg('sexCombo', Ext.form.BmSex);

/**
 * \u63cf\u8ff0:\u5b66\u5386 <br>
 */
var degStore = new Ext.data.SimpleStore({
			name : 'degStore',
			url : Ext.projectPath,
			fields : ['id', 'name']
 ...
 * 
 */
Ext.form.BmDegree = Ext.extend(Ext.form.ComboBox, {
			allowBlank : false,
			emptyText : '\u8bf7\u9009\u62e9\u5b66\u5386',
			blankText : '\u4e0d\u80fd\u4e3a\u7a7a',
			fieldLabel : '\u5b66\u5386',
			...
			displayField : "name",
			initComponent : function() {
				Ext.form.BmDegree.superclass.initComponent.call(this);
			}
		})
Ext.reg('degreeCombo', Ext.form.BmDegree);
 ...
 * \u76ee\u524d\u7528\u4e8e\u4eba\u5458\u4fe1\u606f
 */
var specialtyStore = new Ext.data.SimpleStore({
			name : 'specialtyStore',
			url : Ext.projectPath,
			fields : ['id', 'name']
 ...
 * \u76ee\u524d\u7528\u4e8e\u4eba\u5458\u4fe1\u606f
 */
Ext.form.BmSpecialty = Ext.extend(Ext.form.ComboBox, {
			fieldLabel : '\u4e13\u4e1a',
			allowBlank : false,
			emptyText : '\u8bf7\u9009\u62e9\u4e13\u4e1a',
			blankText : '\u4e0d\u80fd\u4e3a\u7a7a',
			...
			displayField : "name",
			initComponent : function() {
				Ext.form.BmSpecialty.superclass.initComponent.call(this);
			}
		})
Ext.reg('specialtyCombo', Ext.form.BmSpecialty);
 ...
 * \u76ee\u524d\u7528\u4e8e\u4eba\u5458\u4fe1\u606f
 */
var identityStore = new Ext.data.SimpleStore({
			name : 'identityStore',
			url : Ext.projectPath,
			fields : ['id', 'name']
...
Ext.form.BmIdentity = Ext.extend(Ext.form.ComboBox, {
			fieldLabel : '\u7f16\u5236',
			allowBlank : false,
			emptyText : '\u8bf7\u9009\u62e9\u7f16\u5236',
			blankText : '\u4e0d\u80fd\u4e3a\u7a7a',
			...
			displayField : "name",
			initComponent : function() {
				Ext.form.BmIdentity.superclass.initComponent.call(this);
			}
		})
Ext.reg('identityCombo', Ext.form.BmIdentity);
 ...
 * \u76ee\u524d\u7528\u4e8e\u4eba\u5458\u4fe1\u606f
 */
Ext.form.BmJob = Ext.extend(Ext.form.ComboBox, {
			fieldLabel : '\u804c\u52a1',
			allowBlank : false,
			emptyText : '\u8bf7\u9009\u62e9\u804c\u52a1',
			blankText : '\u4e0d\u80fd\u4e3a\u7a7a',
			...
			displayField : "name",
			initComponent : function() {
				Ext.form.BmJob.superclass.initComponent.call(this);
			}
		})
Ext.reg('jobCombo', Ext.form.BmJob);

/**
 * \u63cf\u8ff0:\u6587\u4ef6\u5bc6\u7ea7 <br>
 */
var wjmjStore = new Ext.data.SimpleStore({
	name : 'wjmjStore',
	fields : ['id', 'name'],
	data : system.persistent.wjmj
		// from states.js
	});
var wjmjCxStore = new Ext.data.SimpleStore({
	name : 'wjmjCxStore',
	fields : ['id', 'name'],
	data : system.persistent.wjmj
		// from states.js
 ...
 * \u76ee\u524d\u7528\u4e8e\u4eba\u5458\u4fe1\u606f<br>
 */
Ext.form.BmWjmj = Ext.extend(Ext.form.ComboBox, {
			allowBlank : false,
			emptyText : '\u8bf7\u9009\u62e9\u6587\u4ef6\u5bc6\u7ea7',
			blankText : '\u4e0d\u80fd\u4e3a\u7a7a',
			fieldLabel : '\u6587\u4ef6\u5bc6\u7ea7',
			...
			displayField : "name",
			initComponent : function() {
				Ext.form.BmWjmj.superclass.initComponent.call(this);
			}
		})
Ext.reg('wjmjCombo', Ext.form.BmWjmj);
 ...
 * \u76ee\u524d\u7528\u4e8e\u6587\u4ef6\u67e5\u8be2<br>
 */
Ext.form.BmWjmjCx = Ext.extend(Ext.form.ComboBox, {
			allowBlank : false,
			emptyText : '\u8bf7\u9009\u62e9\u6587\u4ef6\u5bc6\u7ea7',
			blankText : '\u4e0d\u80fd\u4e3a\u7a7a',
			fieldLabel : '\u6587\u4ef6\u5bc6\u7ea7',
			...
			displayField : "name",
			initComponent : function() {
				Ext.form.BmWjmjCx.superclass.initComponent.call(this);
			}
		})
Ext.reg('wjmjCxCombo', Ext.form.BmWjmjCx);



/**
 * @description: \u5728\u5217\u91cc\u663e\u793a\u4e0b\u62c9\u6846\u7684\u503c\u548c\u5728\u5217\u7684store\u4e2d\u4ed8\u503c\u865a\u793c\u540d\u79f0\u5217\u503c<br>
 * @version:2010042301
 * @author:\u80e1\u65b9\u5747
 * @param: <br>
 *         store :\u4e0b\u62c9\u6846\u7684\u9009\u9879\u6570\u636e <br>
 *         storeName: \u5728store\u4e2d\u5b9a\u4e49\u7684\u540d\u79f0<br>
 *         combosId\uff1a\u4e0b\u62c9\u6846\u7684Id<br>
 *         record:\u5f53\u524d\u8bb0\u5f55<br>
 *         value:\u5f53\u524d\u503c<br>
 * 
 * 
 */
function colDisplayCombos(store, combosId, storeName, record, value) {
	// \u83b7\u53d6\u5f53\u524did="combo_process"\u7684comboBox\u9009\u62e9\u7684\u503c
	if (Ext.getCmp(combosId) == null) {
		Ext.Msg.alert('\u63d0\u793a', '\u627e\u4e0d\u5230id\u4e3a"' + combosId + '"\u7684\u9009\u62e9\u6846,\u5c06\u663e\u793a\u4e0d\u6b63\u5e38!')
		return combosId;
	}
	var index = store.find(Ext.getCmp(combosId).valueField, value);
	var temRecord = store.getAt(index);
	var DisplayText = "";
	...
	if (temRecord == null) {
		...
		DisplayText = temRecord.data.name;// \u83b7\u53d6record\u4e2d\u7684\u6570\u636e\u96c6\u4e2d\u7684process_name\u5b57\u6bb5\u7684\u503c
		record.data[storeName] = DisplayText
	}
	return DisplayText;
};
/**
 * convertRecordsToJson(items, store)<br>
 * \u63cf\u8ff0:\u8be5\u51fd\u6570\u7528\u4e8e\u5c06\u7f16\u8f91\u8fc7\u7684Grid\u4e2d\u7684\u6570\u636e\u884c\u5bf9\u5e94\u7684Store\u4e2d\u7684\u6570\u636e\u9879\u8f6c\u6362\u6210JSON <br>
 * \u53c2\u6570:<br>
 * items \u88ab\u4fee\u6539\u7684\u8bb0\u5f55\u6570\u7ec4 <br>
 * store store <br>
 */
function convertRecordsToJson(items, store) {
	/*
	 * if (items.length == 0) { Ext.Msg.alert('\u63d0\u793a', '\u6ca1\u6709\u4ec0\u4e48\u9700\u8981\u4fdd\u5b58\u7684!') return ''; }
	 */
	...
	for (i = 0; i < items.length; i++) {
		...
		for (j = 0; j < fields.length; j++) {
			var fieldName = fields.get(j).name;/* \u5b57\u6bb5\u540d\u79f0 */
			var fieldType = fields.get(j).type;/* \u5b57\u6bb5\u7c7b\u578b */
			var value = '';
			switch (fieldType) {
				case 'date' :
					var rq = new Date(items[i].get(fieldName));
					var fieldFormat = fields.get(j).dateFormat;
					value = rq.format(fieldFormat)
					break;
				default :
					value = items[i].get(fieldName);
			}
...
/**
 * tszf(str)<br>
 * \u63cf\u8ff0:\u9a8c\u8bc1\u662f\u5426\u5305\u542b\u7279\u6b8a\u5b57\u7b26\uff0c\u7a0b\u5e8f\u5141\u8bb8\u5305\u542b\u6570\u5b57\u548c26\u4e2a\u82f1\u6587\u5b57\u6bcd\u7ec4\u6210\u7684\u5b57\u7b26\u4e32
 */
function tszf(str) {
