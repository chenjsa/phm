...
/**
 * Service object for domain model class RadarUserInfo.
 * @see com.pms.rcm.radar.vo.RadarUserInfo
 * @author bao.zhou
 */
@Service("radarUserInfoManager")
public class RadarUserInfoManager extends BaseManager<RadarUserInfo>{  
   	/**
	 * 
	 * <li>方法名：get
	 * <li>@param id
	 ...
	 */
    @Override
	public RadarUserInfo get(Serializable id) throws BusinessException {
		RadarUserInfo entity = super.get(id); 
		return entity;
	}
	...
	public List<RadarUserInfo> findByIds(String[] ids) throws BusinessException{
		if(ids.length ==1 && ids[0].indexOf(",") != -1){
			ids = ids[0].split(",");
		}
		StringBuffer sb = new StringBuffer();
		sb.append("from RadarUserInfo where id in(''");
		for (String id : ids) {
			sb.append(",'" + id + "'");
		}
		sb.append(")");
	 ...
	 * <li>修改日期：
	 */
    public List<RadarUserInfo> findPage(RadarUserInfo entitySearch, Page page,
		...
		if(entitySearch.getProvinceId()!=null){
			sb.append(" and w.provinceId='" + entitySearch.getProvinceId() + "'");
		}
   ...
   @Override
   @Transactional(rollbackFor = { Exception.class })
	public RadarUserInfo  update(RadarUserInfo entity) throws BusinessException {
	 ...
	 */
	@Transactional(rollbackFor = { Exception.class })
	public void remove(RadarUserInfo entity) {
