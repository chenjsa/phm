...
public class ProcessDiagramGenerator {
	 ...
	 */
	public static InputStream generatePngDiagram(BpmnModel bpmnModel,ProcessDefinitionEntity processDefinition) {
		return generateDiagram(bpmnModel,processDefinition, "png", Collections.<String> emptyList());
	}
	 ...
	 * diagram interchange information of the process.
	 */
	public static InputStream generateJpgDiagram(BpmnModel bpmnModel,ProcessDefinitionEntity processDefinition) {
		return generateDiagram(bpmnModel,processDefinition, "jpg", Collections.<String> emptyList());
	}
	 ...
	 */
	   //获取流程图      
	public static InputStream generateDiagram(BpmnModel bpmnModel,ProcessDefinitionEntity processDefinition, String imageType, List<String> highLightedActivities) {
		// 通过流程图画布绘制出对应图像类型的流程图图片
		   //获取流程图    
		return generateDiagram(bpmnModel,processDefinition, highLightedActivities).generateImage(imageType);
	}
	 protected interface ArtifactDrawInstruction {
	   ...
	   protected static Map<Class<? extends BaseElement>, ArtifactDrawInstruction> artifactDrawInstructions = new HashMap<Class<? extends BaseElement>, ArtifactDrawInstruction>();
	 ...
	 * @Version: [v2.0.0]
	 */
	protected static ProcessDiagramCanvas generateDiagram(BpmnModel bpmnModel,ProcessDefinitionEntity processDefinition, List<String> highLightedActivities) {
		...
		for (ActivityImpl activity : processDefinition.getActivities()) {
			drawActivity(bpmnModel,processDiagramCanvas, activity, highLightedActivities);
		}
		...
		for (int index = 1; index <= highLightedActivities.size(); index++) {
			for (ActivityImpl activity : processDefinition.getActivities()) {
				if (highLightedActivities.get(index-1).equals(activity.getId())) {
					drawActivityFlowHighLight(bpmnModel,processDiagramCanvas, activity, highLightedActivities, index);
					// 最后一个节点红色显示
	 ...
	 * @Version: [v2.0.0]
	 */
	protected static void drawActivityFlowHighLight(BpmnModel bpmnModel,ProcessDiagramCanvas processDiagramCanvas, ActivityImpl activity,List<String > highLightedActivities, int index) {
		...
		for (ActivityImpl nestedActivity : activity.getActivities()) {
			drawActivity(bpmnModel,processDiagramCanvas, nestedActivity, highLightedActivities);
		}
	}
	...
	protected static void drawActivity(BpmnModel bpmnModel,ProcessDiagramCanvas processDiagramCanvas, ActivityImpl activity, List<String> highLightedActivities) {
		// 获取节点类型
		...
		for (PvmTransition sequenceFlow : activity.getOutgoingTransitions()) {
			...
			for (int i = 2; i < waypoints.size(); i += 2) { // waypoints.size() // minimally 4: x1, y1, // x2, y2
		        ...
		        if (labelGraphicInfo != null) {
		          processDiagramCanvas.drawLabel(sequenceFlow.getProperty("name").toString(), labelGraphicInfo, false);
		        }
		        else {
		        	GraphicInfo lineCenter = getLineCenter(graphicInfoList);
		        	processDiagramCanvas.drawLabel(sequenceFlow.getProperty("name").toString(), lineCenter, false); 
		        	}
			}
		}
