...
$.formValidator = 
{
	...
	{
	    var tip = $("#"+tipid);
	    var wtip = tipid.substr(0,tipid.length-3);
		if(showmsg==null || showmsg==""){
			tip.hide();
		}else
		{
		
			//设置提示框显示到出错控件的上方 2008-11-03 曾锤鑫
			var element = document.getElementById(wtip);
			var offsetTop = element.offsetTop + element.offsetHeight-5; 
	...
	pageIsValid : function (validatorgroup)
	{
		...
		else
		{
			...
			//if(thefirstid!=""){$("#"+thefirstid).focus();}//2010-11-04曾明辉解决不显示错误提示
		}
	...
	compareValid : function(returnObj)
	{
		...
		//zengchuixin update for 2008-11-22
        }else if(setting.datatype=="date"){
        	if (curvalue != '' && ls_data != '') {
       			var DateOne = ls_data;
       			var DateTwo = curvalue;
				...
				ls_data = Date.parse(OneMonth+"/"+OneDay+"/"+OneYear);
				curvalue = Date.parse(TwoMonth+"/"+TwoDay+"/"+TwoYear);
			}else{
			    return;
			}
        }
